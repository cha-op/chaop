...
App-server command execution only runs against a Chaop thread/task attached to a local app-server Host Session. The Worker includes the attached local session id and cwd in `command.dispatch`, and the connector resolves the session to the current app-server thread id before calling `thread/resume` and `turn/start` from that cwd. Command setup can paginate through the app-server thread list under `codex_timeout_seconds`; cancellation or timeout best-effort sends `turn/interrupt` after a turn id is known. Chaop records lifecycle events and the final assistant message summary only; local commandExecution output is not uploaded by default.
