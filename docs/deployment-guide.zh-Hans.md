...
只有本机 Codex CLI 确实需要时才加入可选设置。`execution.extra_args` 只用于 `codex exec` fallback flags；`codex app-server` 可用的 flags 请放在 `session_inventory.managed_app_server.extra_args`。`app_server` 和 private `codex_exec` fallback 都可能消耗 Codex/OpenAI 额度或 API budget；让真实执行无人值守运行前，请先按 [成本模型](cost-aware.zh-Hans.md) 设置告警。
