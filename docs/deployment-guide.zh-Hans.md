...
- 如果 app-server execution 在 turn 开始前失败，检查 `execution.mode = "app_server"`、`session_inventory.app_server_url` 是否已设置、目标 Chaop thread 是否已经 attach 到 app-server Host Session，以及 app-server 是否正在配置的 listener 上运行。对于已认证 listener，还要确认 `session_inventory.app_server_auth_token_file` 指向 `--ws-token-file` 使用的同一个非空 token。
...
- 如果 New local thread 返回 app-server 错误，检查 `codex app-server --listen ws://127.0.0.1:9876` 是否正在运行、`session_inventory.app_server_url` 与 capability-token file 是否匹配，以及修改配置后是否已经重启 connector。
