...
当 Chaop 需要让这个 connector 管理一个专用的本机 Codex app-server listener 时，设置 `session_inventory.managed_app_server.enabled = true`。如果 listener 不存在，connector 会带上配置里的 `execution.codex_profile`、`execution.codex_model`、`session_inventory.managed_app_server.extra_args` 和 `--listen <listen_url>` 启动 `codex app-server`；声明 app-server capabilities 前会先做 protocol 初始化级别的健康检查；如果子进程退出或启动失败，会在 `restart_backoff_seconds` 后重试。在 Unix 主机上，应优先使用位于私密目录中的绝对 `unix:///path/to/private/app-server.sock` URL，并确保 model-invoked tools 无法访问该目录。Connector 也接受绑定到 `localhost`、`127.0.0.1` 或 `::1` 的 WebSocket listener，并会拒绝非 loopback host，避免把 app-server protocol 暴露到 LAN interface。Loopback 本身不是 authentication boundary：connector 不会通过明文 `ws://`（包括 loopback）发送 app-server Bearer token。Managed mode 不能与 `session_inventory.app_server_auth_token_file` 同时使用；该 token 只用于由外部管理且经过证书验证的 `wss://` listener。外部 listener 的私密 token file 必须位于 workspace 之外并对 tools deny。如果你已经用其他 service manager 管理这个 listener，则保持 managed mode 关闭，并设置 `session_inventory.app_server_url` 指向外部 listener。
