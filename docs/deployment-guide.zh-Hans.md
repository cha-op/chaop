...
部署 profile 保存部署实例值，例如域名、资源名、D1/R2 绑定，以及 Access 配置。deploy token 保存在私有 ops 仓的 `secrets/` 目录下，只传给当前 Wrangler 进程。脚本会在 `.codex-tmp/deploy/api/` 下写入临时 Wrangler 配置，执行远端 D1 migration，然后部署 API Worker。
...
生成的 Worker runtime vars 也会在可用时包含这些非 secret 的 Cloudflare telemetry selector：
...
Cloudflare telemetry 是可选功能。要启用它，请单独创建一个只读 Cloudflare API token，并授予这个 account 权限：
...
# 本机 Unix socket；不要设置 app-server auth token：
# app_server_url = "unix:///path/to/private/app-server.sock"
# 或同时使用下面两行连接经过证书验证的外部 listener：
...
当 Chaop 需要让这个 connector 管理一个专用的本机 Codex app-server listener 时，设置 `session_inventory.managed_app_server.enabled = true`。如果 listener 不存在，connector 会带上配置里的 `execution.codex_profile`、`execution.codex_model`、`session_inventory.managed_app_server.extra_args` 和 `--listen <listen_url>` 启动 `codex app-server`；声明 app-server capabilities 前会先做 protocol 初始化级别的健康检查；如果子进程退出或启动失败，会在 `restart_backoff_seconds` 后重试。在 Unix 主机上，应优先使用位于私密目录中的绝对 `unix:///path/to/private/app-server.sock` URL，并确保 model-invoked tools 无法访问该目录。Connector 也接受绑定到明确 loopback IP 地址 `127.0.0.1` 或 `::1` 的 WebSocket listener，并会拒绝 hostname 和非 loopback address，避免把 app-server protocol 暴露到 LAN interface。Loopback 本身不是 authentication boundary：connector 不会通过明文 `ws://`（包括 loopback）发送 app-server Bearer token。Managed mode 不能与 `session_inventory.app_server_auth_token_file` 同时使用；该 token 只用于由外部管理且经过证书验证的 `wss://` listener。外部 WebSocket connection 会对 DNS、TCP、TLS 与 WebSocket handshake 应用有界 timeout，并且不跟随 redirect，因此 Authorization header 不会被转发到其他 origin 或降级 transport。外部 listener 的私密 token file 必须位于 workspace 之外并对 tools deny。如果你已经用其他 service manager 管理这个 listener，则保持 managed mode 关闭，并设置 `session_inventory.app_server_url` 指向外部 listener。

在 managed mode 下，只有 managed app-server URL 健康到足以初始化 protocol 时，connector 才会声明 `app_server_threads` 和 `app_server_archive`；只有该 URL 健康且 `execution.mode = "app_server"` 时，connector 才会声明 `codex_app_server_exec` 和 `host_session_app_server_ensure`。Connector 连接 Worker 后会通过 `agent.ready` 刷新这些 capabilities，所以 degraded managed listener 不会继续被选中执行新的 app-server 工作。如果使用外部管理的 `session_inventory.app_server_url`，capability 声明会跟随配置的 URL，listener 的健康维护由外部 service manager 负责。这和只适用于 CLI 的 `codex_exec` capability 是分开的。只有同一台在线 workspace connector 同时声明 `app_server_threads` 和 `codex_app_server_exec` 时，Worker 才接受新建本机 thread 请求；Web connector 选择也使用相同要求。只声明 `app_server_threads` 的健康 listener 在这个流程中仅用于 inventory，因此使用本机 thread 创建前，需要在同一 connector 上设置 `execution.mode = "app_server"`。已 attach 的 app-server command execution 要求 owning connector 声明 `codex_app_server_exec`；Chaop 会拒绝 command creation 或 lease，而不是 fallback 到另一台 connector 的 `codex_exec`。Host Session attach 只有在 connector 声明 `host_session_app_server_ensure` 时，才会发送 app-server resume/ensure control message；较旧的 app-server execution connector 会保留 D1-only attach 路径，不会因为未知 control envelope 等到 timeout。Connector 不声明 `app_server_archive` 时，archive/unarchive 会保持 D1-only；声明后，Chaop 会先更新 D1，然后 connector 把存储的 Codex session id 解析成 app-server thread id，再调用 `thread/archive` 或 `thread/unarchive`。同步失败会作为 Browser warning 回传，不会阻止本地 archive 状态更新。`app_server_timeout_seconds` 应保持较短，避免 app-server 停止时让 connector 启动、thread 创建、command setup、Host Session attach 或 archive 同步等待过久。
...
新建本机 thread 一律使用 connector 配置里的 `workspace_root` 作为启动 cwd；Browser API 不接受也不会转发任意 cwd。对于专用 workspace-write connector，设置 `lock_cwd_to_workspace_root = true` 后，所有 managed `thread/resume` 与 `turn/start` 都会强制回到该根目录，包括记录 cwd 位于其他嵌套目录的已 attach Host Session。这样 project-instruction discovery 会固定在受保护的根部文件；只有在确实需要保留每个 attached session cwd 时才保持为 false。
...
如果不使用 managed mode，优先在 workspace 外使用私有 Unix socket：
...
日常 dogfood 时，请使用已提交的 operator script 来运行常驻 connector；详细说明见 [Dogfood Connector 运行手册](dogfood-runbook.zh-Hans.md)：
...
脚本默认把 PID 和 log 状态保存在 `${XDG_STATE_HOME:-$HOME/.local/state}/chaop/dogfood/`，而不是 `/tmp`。

如果只跑一个窄范围 smoke test，使用：

```bash
pnpm dogfood:connector -- once
```

如果只是临时开发，不需要 persistent wrapper，仍然可以直接运行原始 connector command：
...
- 如果 app-server execution 在 turn 开始前失败，检查 `execution.mode = "app_server"`、`session_inventory.app_server_url` 是否已设置、目标 Chaop thread 是否已经 attach 到 app-server Host Session，以及 app-server 是否正在配置的 listener 上运行。对于已认证的外部 listener，还要确认 URL 使用经过证书验证的 `wss://`，且 `session_inventory.app_server_auth_token_file` 指向预期的非空 token。
...
- 如果 New local thread 返回 app-server 错误，检查配置的 Unix socket 是否存在于私密目录中、`session_inventory.app_server_url` 是否与之匹配，以及修改配置后是否已经重启 connector。WebSocket 部署还需确认 listener 与 URL 的 scheme、host 和 port 一致；Bearer token 会被拒绝通过明文 `ws://` 发送。
