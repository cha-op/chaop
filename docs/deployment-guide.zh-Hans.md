...
- 如果 New local thread 返回 app-server 错误，检查 `codex app-server --experimental --listen ws://127.0.0.1:9876` 是否正在运行、`session_inventory.app_server_url` 是否匹配，以及修改配置后是否已经重启 connector。
