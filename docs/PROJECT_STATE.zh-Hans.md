...
- Rust connector 会上报轻量本机 Codex session inventory，并可以可选使用 app-server `Thread.name` 做 title enrichment。
...
## 全局阻塞项
- Experimental Codex app-server execution integration 和 R2 artefact capture 仍然放在后续切片。
