...
- [todo] 确认 Codex app-server 对不在 `thread/list` 中的历史 rollout session 是否有正式 resume-by-session-id protocol；在确认之前，Chaop attach 只提升可解析到 app-server thread 的 sessions。
