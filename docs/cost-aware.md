...
- Low-cost D1 write activity signals: measured current-day D1 writes, schema-model estimated persisted-event writes from the current daily usage window, and the residual gap between the two. This is an activity signal, not exact per-query attribution.
