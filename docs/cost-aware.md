...
- Managed app-server health checks are local connector probes. The connector now reports AppServerInstance state through a dedicated channel, with unchanged healthy summaries filtered by Durable Object in-memory dedupe and a 15 minute D1 debounce. State edges and active turn count changes still persist promptly.
