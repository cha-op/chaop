...
- The server-side dogfood safety guard uses the same short-cached live Cloudflare telemetry query as the Budget Board, but does not persist telemetry samples from guarded action paths. Persisted telemetry history remains a Budget Board read path.
