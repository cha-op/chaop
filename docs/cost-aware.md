# Cost Governance Source Note [ British English | 简体中文 ]

## British English

This file is a source cost-governance note, not the final user-facing cost model. The user-facing cost model should be maintained bilingually as the implementation matures; for now, `docs/deployment-guide.md` and `docs/PROJECT_TODO.md` track the deployment values and follow-up documentation work.

## 简体中文

本文是来源成本治理笔记，不是最终面向用户的成本模型。面向用户的成本模型应随实现成熟继续维护为双语文档；当前由 `docs/deployment-guide.md` 和 `docs/PROJECT_TODO.md` 记录部署值和后续文档工作。
