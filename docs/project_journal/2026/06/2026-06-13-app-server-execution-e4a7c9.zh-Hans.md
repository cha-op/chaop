...
- 对该修复的最后 review 发现 agent 在 `thread/list` 返回 error-like response、response 前关闭，或暴露分页时，仍可能把不完整 app-server inventory 标记为 full。现在 app-server Host Session inventory 会把 JSON-RPC errors、malformed responses 和 early close/reset 当作失败，会把 paginated 或被 `max_sessions` 截断的 Host Session report 标记为 incremental，并把深分页留给有自己预算的 command/archive resolvers。
- 最终 rerun reviews 又发现两个完整性缺口：schema drift 的 app-server row 或 cursor 仍可能被接受为成功的 full inventory；Worker 会把旧版缺省 `inventory_scope` 或缺少 app-server inventory 证据的 full report 当成足以清理 omitted sessions 的证据，同时只清理 title 来自 app-server 的行。现在 agent 会拒绝缺少可执行 thread `id` 的 thread/list row、非字符串或空 cursor；Worker 会把缺省 scope 当作 incremental，要求显式 `app_server_inventory_ok: true` 才能执行 omitted-session cleanup，并按当前 `app_server_present` 状态而不是 title source 清理 omitted sessions。
...
- 重新运行的 offline frozen-diff review 发现 app-server inventory 每次 report 都可能翻完整个 app-server history，尤其是大量 threads 共享同一个 session tree 时。现在 connector 只会请求一页有界的 `max_sessions + 1` 条 raw threads，任何 paginated 或 over-limit response 都会标记为 incremental；Worker 也只会在 inventory 同时 full 且 successful 时，才用 report 里的 app-server absence demote 已保存的 presence。
...
- Rust tests 断言 app-server Host Session inventory 会拒绝 malformed rows、缺少可执行 thread id 的 rows 和 malformed cursors，遇到 `thread/list` error 或 malformed response 会失败，并且 disabled、paginated 或 truncated Host Session report 会标记为 incremental。
- Rust tests 断言 app-server Host Session inventory 使用一页有界的 `max_sessions + 1` raw-thread request，不会继续跟随 paginated 或 duplicate-session pages，并在成功截断时保留 `app_server_inventory_ok: true`。
