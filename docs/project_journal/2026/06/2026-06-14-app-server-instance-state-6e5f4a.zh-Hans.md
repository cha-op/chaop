...
- Worker 会把 connector-wide app-server instance state 持久化到 D1，并对未变化的 healthy reports 做服务端去重和 debounce。
- Connector 会及时上报状态边缘，并用低频节奏发送未变化的 healthy summaries。
...
- 增加 Worker DB recording，包含 15 分钟 unchanged healthy debounce、state-edge 立即写入、snapshot omission 标记 stopped，以及 connector offline 标记 stopped。
- 增加 Durable Object validation、ack、browser fanout，以及短窗口内存级 duplicate healthy report suppression。
- 增加 connector 侧 app-server instance snapshots、`app_server_instance_state` capability、ack/retry handling、五分钟 healthy summaries，以及 app-server turn 前后的 active turn count 上报。
...
- 即使 DO cache miss，D1 仍会对未变化 healthy summaries 做 15 分钟 debounce，避免高频写入循环。
- Degraded、restarting、stopped 等状态边缘和 active turn count changes 会绕过 healthy debounce。
