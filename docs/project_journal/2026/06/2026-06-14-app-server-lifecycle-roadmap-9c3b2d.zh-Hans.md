...
- Review hardening 让 bootstrap 不再额外发起 connector/task budget-state aggregate queries，legacy unknown metric source 会明确显示为未知来源，并且 API 会保留超过 100% 的 over-budget 百分比，只在 UI meter control 上单独 clamp。
