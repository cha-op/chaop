...
- 最终 reload helper preflight 在卸载健康 service 前验证了 release binary 与 connector configuration，检查 configured Unix listener 的路径长度与 parent-directory writability，在有长度上界的私有 socket 上启动 candidate Codex app-server、完成 protocol initialisation 并确认其正常终止，同时检查了已安装 plist 与精确的 passwordless lifecycle permissions。终止失败时会保留 child handle 供 cleanup retry。Candidate 只继承从已安装 plist 读取的 working directory、`HOME`、`PATH` 与 umask。随后一次真实 system-domain reload 成功完成：connector process 只启动一次且保持稳定、没有历史退出，并通过了 protocol-level app-server health check。
