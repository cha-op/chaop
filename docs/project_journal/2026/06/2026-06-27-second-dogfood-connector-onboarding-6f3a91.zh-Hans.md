...
- 专用 connector 现在可以把所有 managed resume、archive fallback 与 turn 锁定到配置的 workspace root。配合根部 `AGENTS.md` 和 `AGENTS.override.md` 的精确只读规则，可避免 writable nested instruction file 进入后续由 Chaop 创建的 thread discovery。Connector 配置还会拒绝把 managed listener 与外部 listener bearer-token field 同时启用。
...
- App-server connection setup 现在对 DNS、TCP、TLS、WebSocket 与 Unix-socket setup 使用同一个绝对 deadline。可取消的 socket watchdog 会在该 deadline 停止 slow-drip handshake。DNS 使用一个 active、capacity-bounded resolver worker；发生 timeout 或 disconnect 的 generation 会被隔离并替换，同时最多保留两个 retired workers，防止重复的 system-resolver stall 无界累积 threads。Unix control descriptor 已设置 close-on-exec：支持的平台使用原子 socket flag，Apple targets 使用可移植的 descriptor fallback。
- 在未认证阶段、capability-token 认证阶段以及最终 Unix-socket 迁移后，真实 managed turns 都已通过部署后的 Chaop command path 创建 ignored canary，并且无需 profile 外 approval 即进入 `command.finished`。Canary content 与 mode 已验证，turn 没有修改 tracked files，随后也已删除 canaries。最终改动通过了 48 个 script、3 个 protocol、61 个 Web、294 个 Worker 与 186 个 Rust tests，以及 production 与 release connector builds。
