...
- 连续冻结提交范围独立审查共发现六个边界：已部署 binary 可写、credential pattern 覆盖不完整、runtime root 扩大写权限、未认证 loopback control listener、isolated-review state 可写，以及明文 listener 竞态中的 bearer-token 暴露。最终 profile 使用固定绝对 write root，保护 binary 与 credential families，deny `.codex-tmp` 与私有 connector-state directory，并通过私有 Unix socket 控制本机 app-server。后续 ops review 还发现过宽的 `.dev.vars.*` deny 同时阻断了已跟踪的占位模板；规则已收窄到明确的 credential variants，精确 profile probe 验证了模板可写，而 `.dev.vars.local` 与私有 socket 仍不可用。重载后的 LaunchDaemon 已在运行中 app-server arguments 里暴露最终 profile 与 Unix endpoint，且一次部署后 control-plane turn 已通过该 socket 完成。
