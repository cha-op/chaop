...
- Managed app-server 现在使用固定到 Chaop repository 的 opt-in workspace-write permission profile。Git metadata、repository policy 与 skills、project Codex configuration、GitHub workflows、常见 credential files、动态 workspace roots 与通用 home-directory reads 均保持在 write boundary 之外。Build 使用专用私密 temporary directory，Cargo dependency cache 保持 offline，test network 仅允许 loopback。
