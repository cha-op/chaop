...
- 最终 reload helper preflight 在卸载健康 service 前验证了 release binary 与 connector configuration，在隔离 socket 上启动 candidate Codex app-server 并完成 protocol initialisation，同时检查了已安装 plist 与精确的 passwordless lifecycle permissions。随后一次真实 system-domain reload 成功完成：connector process 只启动一次且保持稳定、没有历史退出，并通过了 protocol-level app-server health check。
