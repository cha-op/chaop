...
- 专用 connector 现在可以把所有 managed resume、archive fallback 与 turn 锁定到配置的 workspace root。配合根部 `AGENTS.md` 和 `AGENTS.override.md` 的精确只读规则，可避免 writable nested instruction file 进入后续由 Chaop 创建的 thread discovery。Connector 配置会拒绝把 managed listener 与外部 listener bearer-token field 同时启用；外部 token file 只有在配置的 app-server URL 使用 `wss://` 时才会被接受，并且从配置到连接建立的 URL scheme 检查均统一为大小写不敏感。
