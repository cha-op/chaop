...
- Review follow-up 也会在 managed command `turn/start` 和 archive/unarchive sync 前校验 rollout-path app-server `thread/resume` response，避免 stale 或损坏的 rollout path 在另一个 app-server session 上运行 turn 或修改 archive state。
