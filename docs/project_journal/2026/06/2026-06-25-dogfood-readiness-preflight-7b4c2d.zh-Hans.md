...
- 已选择且已 attach 的 thread 现在只接受拥有其 app-server Host Session 的 connector 所报告的 health；如果 active attachment 因 detached、offline 或超出有界 inventory 而不可见，readiness 会 blocked 并导向 Host Sessions，不会断言该 thread 从未 attach，也不会借用 workspace-level capacity 后落入 placeholder execution。
- 如果 owning connector 缺失、失去 workspace 关联、离线或缺少 app-server execution capability，connector check 会直接报告 owner 的实际问题，而不是诊断同一 workspace 中的其它 connector。
