...
- Frozen-range independent reviews identified six boundaries in successive profiles: a writable deployed binary, incomplete credential-pattern coverage, runtime-root write expansion, an unauthenticated loopback control listener, writable isolated-review state, and bearer-token exposure to a process that won a plaintext listener race. The final profile uses a fixed absolute write root, protects the binary and credential families, denies `.codex-tmp` and the private connector-state directory, and controls the local app-server through a private Unix socket. A later ops review caught an over-broad `.dev.vars.*` deny that also blocked the tracked placeholder template; the rule was narrowed to explicit credential variants, and an exact profile probe verified that the template is writable while `.dev.vars.local` and the private socket remain unavailable. The reloaded LaunchDaemon exposes the final profile and Unix endpoint in its running app-server arguments, and a deployed control-plane turn completed through that socket.
