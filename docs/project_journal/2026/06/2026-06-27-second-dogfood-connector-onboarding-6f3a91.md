...
- Frozen-range independent reviews identified four boundaries in successive profiles: a writable deployed binary, incomplete credential-pattern coverage, runtime-root write expansion, and an unauthenticated loopback control listener. The final profile uses a fixed absolute write root, protects the binary and credential families, and requires a capability token that managed tools cannot read. The reloaded LaunchDaemon exposes the final profile and auth flags in its running app-server arguments, and a deployed control-plane turn completed through the authenticated connector.
