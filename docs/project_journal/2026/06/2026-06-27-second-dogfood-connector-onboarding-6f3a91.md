...
- Real managed turns created ignored canaries through the deployed Chaop command path and reached `command.finished` without an out-of-profile approval before authentication, with capability-token authentication, and after the final Unix-socket migration. Canary content and mode were verified, tracked files stayed unchanged by the turns, and the canaries were removed afterwards. The final change passed 48 script, 3 protocol, 61 Web, 294 Worker, and 184 Rust tests plus the production and release connector builds.
