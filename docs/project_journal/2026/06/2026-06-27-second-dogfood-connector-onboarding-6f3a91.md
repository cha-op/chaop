...
- The connector now supports an optional app-server capability-token file and adds its value only as a WebSocket Bearer header. The deployed app-server requires that token; its private file is outside the repository and denied to managed tools. Direct probes observed HTTP `401` without the token and a successful protocol initialisation with it.
- Real managed turns created ignored canaries through the deployed Chaop command path and reached `command.finished` without an out-of-profile approval, both before and after app-server authentication was enabled. Canary content and mode were verified, tracked files stayed unchanged by the turns, and the canaries were removed afterwards. The final change passed 48 script, 3 protocol, 61 Web, 294 Worker, and 170 Rust tests plus the production build.
...
- Frozen-range independent reviews identified five boundaries in successive profiles: a writable deployed binary, incomplete credential-pattern coverage, runtime-root write expansion, an unauthenticated loopback control listener, and writable isolated-review state. The final profile uses a fixed absolute write root, protects the binary and credential families, denies `.codex-tmp`, and requires a capability token that managed tools cannot read. The reloaded LaunchDaemon exposes the final profile and auth flags in its running app-server arguments, and a deployed control-plane turn completed through the authenticated connector.
