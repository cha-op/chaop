...
- The managed app-server now uses an opt-in workspace-write permission profile fixed to the Chaop repository. Git metadata, repository policy and skills, project Codex configuration, GitHub workflows, and the deployed connector binary remain read-only. Workspace-relative deny globs cover nested environment, package-manager, and Cargo credential variants; dynamic workspace roots and general home-directory reads remain unavailable. Builds use a dedicated private temporary directory, cached Cargo dependencies stay offline, and test networking is loopback-only.
...
- A frozen-range independent review identified writable deployed-binary and incomplete credential-pattern boundaries in the first profile. The follow-up direct Codex sandbox probe retained ordinary workspace writes while blocking both the deployed-binary write-open and nested credential reads; the reloaded LaunchDaemon exposes the hardened profile in its running app-server arguments.
