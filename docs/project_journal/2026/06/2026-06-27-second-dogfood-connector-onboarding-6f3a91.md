...
- The dedicated connector can now lock every managed resume, archive fallback, and turn to its configured workspace root. Together with exact read-only rules for root `AGENTS.md` and `AGENTS.override.md`, this prevents a writable nested instruction file from entering later Chaop-created thread discovery. Connector configuration also rejects combining a managed listener with the external-listener bearer-token field.
...
- App-server connection setup now applies one absolute deadline across DNS, TCP, TLS, WebSocket, and Unix-socket setup. A cancellable socket watchdog stops slow-drip handshakes at that deadline. DNS uses one active capacity-bounded resolver worker; a timed-out or disconnected generation is isolated and replaced, while a two-retired-worker cap prevents repeated system-resolver stalls from accumulating threads without bound.
- Real managed turns created ignored canaries through the deployed Chaop command path and reached `command.finished` without an out-of-profile approval before authentication, with capability-token authentication, and after the final Unix-socket migration. Canary content and mode were verified, tracked files stayed unchanged by the turns, and the canaries were removed afterwards. The final change passed 48 script, 3 protocol, 61 Web, 294 Worker, and 185 Rust tests plus the production and release connector builds.
