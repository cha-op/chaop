...
- The TCP fallback now applies one absolute connection deadline across DNS, TCP, TLS, and WebSocket setup. A cancellable socket watchdog stops slow-drip handshakes at that deadline, while one shared capacity-bounded resolver worker prevents repeated DNS timeouts from accumulating threads.
- Real managed turns created ignored canaries through the deployed Chaop command path and reached `command.finished` without an out-of-profile approval before authentication, with capability-token authentication, and after the final Unix-socket migration. Canary content and mode were verified, tracked files stayed unchanged by the turns, and the canaries were removed afterwards. The final change passed 48 script, 3 protocol, 61 Web, 294 Worker, and 179 Rust tests plus the production and release connector builds.
