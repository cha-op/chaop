...
- A rerun offline frozen-diff review found app-server inventory could page through the entire app-server history on every report, especially when many threads shared the same session tree. The connector now requests one bounded page of `max_sessions + 1` raw threads, marks any paginated or over-limit response incremental, and Worker only uses reported app-server absence to demote stored presence when the inventory is both full and successful.
...
- Rust tests assert app-server Host Session inventory rejects malformed rows, rows without executable thread ids, and malformed cursors; fails on `thread/list` errors or malformed responses; and marks disabled, paginated, or truncated Host Session reports as incremental.
- Rust tests assert app-server Host Session inventory uses one bounded `max_sessions + 1` raw-thread request, does not follow paginated or duplicate-session pages, and preserves `app_server_inventory_ok: true` when truncation is successful.
