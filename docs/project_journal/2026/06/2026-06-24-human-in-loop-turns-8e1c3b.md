...
- The final readiness review found that dispatch-started claims still needed an explicit recovery path. Worker now keeps ambiguous `sent_unknown` deliveries locked during the short duplicate-prevention window, releases claims only for `not_sent` or connector-rejected responses, and reclaims stale dispatch-started claims after a longer timeout so an interrupted response cannot permanently strand an approval or input request.
- The follow-up review found that ambiguous deliveries need their own durable state, not just a dispatch-started timestamp. Worker now marks `sent_unknown` claims with `delivery_uncertain_at` so they are not TTL-cleaned and re-sent, while still allowing never-sent dispatch-started claims to recover after the longer timeout. Browser input submission also preserves exact answer text, Worker rejects crafted answers outside supplied options unless `is_other` is allowed, and the connector waits up to the Worker delivery timeout for the app-server worker acknowledgement.
...
- Ambiguous `sent_unknown` responses add one more bounded marker write, `delivery_uncertain_at`, only on the explicit operator response path. There is no background retry, poller, or sweep; recovery requires a later explicit operator action or future manual recovery UI.
