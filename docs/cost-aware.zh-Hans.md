...
- Managed app-server health checks 是本机 connector probes。Connector 现在会通过独立通道上报 AppServerInstance state；未变化的 healthy summaries 会先经过 Durable Object 内存去重，再受 15 分钟 D1 debounce 保护。状态边缘和 active turn count changes 仍会及时持久化。
