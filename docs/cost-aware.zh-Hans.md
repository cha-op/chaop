...
- Server-side dogfood safety guard 使用已持久化的 Cloudflare telemetry samples 和本地 usage windows，不会把 live GraphQL query 放到受保护写路径前面。对于当前 UTC 日，受保护写路径会分别读取每个 telemetry metric 已持久化的最大累计 counter，因此后续更低或不完整的 Cloudflare sample 不会放松更早触发的 hard limit。`/api/safety-posture` 仍然是面向 operator 的 safety data 显式 live refresh 路径；它会把刷新到的 sample 写入同一个低频 telemetry bucket，并在返回 safety posture 前把 live sample 和当前日已持久化最大值合并。
...
Worker 会在持久化 thread events 的同类路径里写入这些 windows，包括有界 history backfill inserts。每个持久化 thread event 记为一个 Chaop usage unit；低优先级 P2/P3 events 会增加 delayed counter，`command.output` summaries 会增加 compacted counter，已存储 summary bytes 会增加 local spool byte counter。Usage-window writes 会按 window id 聚合，因此一批导入很多 events 时，通常最多只会分别更新当前 daily、four-hour 和 burst rows 各一次。Backfill 导入的 events 会在 thread history 里保留原始 event timestamp，但 D1 write-budget windows 使用导入时间，因为这些 rows 消耗的是当天的 D1 write 额度。默认 budget thresholds 会按 Cloudflare Free D1 rows-written 额度缩放，并使用保守的 schema-derived write model：Chaop 按每个 event 26 行 D1 预算，对齐带 attached task 且处在 daily usage-window boundary 的 command lifecycle。更便宜的 steady realtime event 会写入 12 行 D1 rows，包括 thread sequence update、event row 和 indexes，以及三个 usage-window updates。边界事件在需要插入新 usage-window row 时会更高：一分钟里的第一个 event 是 14 rows，四小时窗口里的第一个 event 是 16 rows，UTC 日窗口里的第一个 event 是 18 rows。带 attached task 的 command lifecycle event 在 steady case 下是 20 rows，因为它还会更新 command state、task state 和 connector activity。有界 backfill 是每个导入 event 6 rows，加上批量 usage-window updates 的固定开销；同一分钟 import batch 且当前 windows 已存在时，通常再加 6 rows。
...
Cloudflare telemetry 可用时，Worker 对每个 sample bucket 和 telemetry selector 最多持久化一条 `budget_telemetry_samples` row。Selector hash 覆盖 Cloudflare account、API Worker、Web Worker、D1 database 和 Durable Object namespace selector，因此环境切换不会复用旧样本。History 读取只查最近 24 小时，最多 300 rows；除非刚刚插入了新 sample row，否则会按默认 60 秒在 isolate 内缓存。斜率计算只使用与最新样本同一个 UTC 日内的样本，因为 Cloudflare 的 D1 rows-written counter 是当前日累计值，并在 00:00 UTC 重置。受保护写入检查也保持在同一个 UTC 日重置边界内，但会按 metric 聚合已持久化的 daily maximum，而不是只信任最新 bucket。D1 activity residual 可以帮助定位非 event 写入来源，但精确 attribution 仍需要后续在写路径外包一层 query meta 记录。
...
新 connector 的第一次 inventory 最多可以创建 `session_inventory.max_sessions` 条 Host Session 记录。D1 会计算 table 和 index mutation，因此这次一次性导入可能表现为陡峭的 rows-written 峰值，让短窗口 projection 远高于后续 idle rate。之后没有变化的 report 会跳过 Host Session row update，但仍会更新一条 sync summary 和 connector activity timestamp。更新时间相同的 sessions 会在 top-N 截断前使用 session id 作为稳定 tie-breaker，因此 connector restart 不会导入另一组随机 subset。这个限制约束的是单次 report，而不是 database retention 上限：recent top-N 窗口真正变化时，新出现的 sessions 会继续加入，较旧的 unattached rows 则保留。在把 inventory storage 视为永久有界之前，需要增加明确的 stale unattached-session retention。
