...
- 低成本 D1 write activity signals：Cloudflare 测得的当前日 D1 writes、从当前 daily usage window 和 schema write model 推导出的 persisted-event writes 估计值，以及两者之间的 residual gap。这是 activity signal，不是精确的 per-query attribution。
