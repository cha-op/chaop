...
- Rust connector crate，可以连接 Worker、接收 command dispatch，默认执行 placeholder，也可以通过私有配置显式开启 managed Codex app-server 或本机 private `codex exec` 执行。
- 初始 D1 schema migration 集合：`migrations/d1/`。

当前切片已经会把 command lifecycle 写入 D1，通过 Durable Object relay pending command，用 Rust connector 跑完闭环，attach 本机 Codex sessions，可以创建新的本机 Codex app-server thread，并且可以通过 dogfood connector 运行 managed app-server turns。本机 Codex CLI 执行需要按 connector 显式设置 `execution.mode = "codex_exec"`；R2 artefact capture 仍然是后续切片。
...
- `docs/dogfood-runbook.zh-Hans.md`
