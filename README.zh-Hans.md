...
当前切片已经会把 command lifecycle 写入 D1，通过 Durable Object relay pending command，用 Rust connector 跑完闭环，attach 本机 Codex sessions，并且可以通过配置了 `session_inventory.app_server_url` 的 connector 创建新的本机 Codex app-server thread。本机 Codex CLI 执行需要按 connector 显式设置 `execution.mode = "codex_exec"`；R2 artefact capture 仍然是后续切片。
